...
import { TextInput, Textarea, Button, Container, Grid, Group, Title, Text, Notification, Divider, Flex } from '@mantine/core';
import { IconMail, IconPhone, IconMapPin, IconBuilding, IconCheck, IconX } from '@tabler/icons-react';
...
const ContactPage = () => {
  ...
  const form = useForm({
    ...
    validate: {
      ...
      lastName: (value) => (value.trim().length < 2 ? 'Last name must have at least 2 characters' : null),
      email: (value) => (/^\S+@\S+$/.test(value) ? null : 'Invalid email'),
      phone: (value) => (/^\d+$/.test(value) ? null : 'Phone number must contain only digits'),
      company: (value) => (value.trim().length < 2 ? 'Company name must have at least 2 characters' : null),
      message: (value) => (value.trim().length < 5 ? 'Message must have at least 5 characters' : null),
    },
  });
    ...
    <Container size="lg" py="xl">
      <Grid gutter="xl">
        ...
        <Grid.Col span={{ base: 12, md: 6, lg: 6 }}>
          <Title order={1} mb="xl">Contact us</Title>
